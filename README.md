...
MetaMask Extension: Installed and initialized in your browser (Download MetaMask).

Local Blockchain Emulator:

Ganache UI (Quickstart Ethereum Workspace).
...
Click the "QUICKSTART" (Ethereum) button to spin up a local 10-account workspace.

Verify the default RPC Server endpoint displayed in the header:
...
Open the MetaMask extension and click Add Network > Add a network manually:
...
Import at least three test accounts from Ganache using their private keys (click the key icon next to any account in Ganache):
...
Create LogisticsEscrow.sol in Remix and paste the contract code.

Compile with Solidity compiler ^0.8.0.
...
Copy the deployed contract address and verify it matches the configuration in app.js:
